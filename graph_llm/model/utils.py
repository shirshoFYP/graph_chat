...
def batch_subgraph_graph_level(
    edge_index,
    node_ids,
    num_nodes,
    graph_size=20,
    ...
    for node_idx in node_ids:
        ...
        node_idx = torch.zeros(
            node_mask.size(0), dtype=torch.long, device=edge_index.device
        )
        node_idx[subset] = torch.arange(
            node_mask.sum().item(), device=edge_index.device
        )
    ...
    subset, mapping, batch = (
        torch.cat(subset_list),
        torch.as_tensor(mapping_list),
        torch.as_tensor(batch_list),
    )
...
def batch_subgraph(edge_index, node_ids, num_nodes, num_hops=3, fans_out=(50, 50, 50)):
