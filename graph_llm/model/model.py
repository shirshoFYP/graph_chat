...
# TODO: Remove redundant code, refactor to PyTorch classes
...
from model.utils import (
    ...
    sample_top_p,
    batch_subgraph_graph_level,
    batch_subgraph_pair_level,
    batch_subgraph,
    add_full_rrwp,
)
...
class LoraInjectedLinear(nn.Module):
    def __init__(
        ...
        self.scale = 1.0 * lora_alpha / lora_r
    ...
    def __init__(self, dim: int, hidden_dim: int, multiple_of: int, args: ModelArgs):
        ...
        if self.params.w_lora:
            if "up_proj" in args.target_modules:
                self.lora_w3 = LoraInjectedLinear(
                    self.w3.in_features,
            ...
            if "down_proj" in args.target_modules:
                self.lora_w2 = LoraInjectedLinear(
                    self.w2.in_features,
            ...
            if "gate_proj" in args.target_modules:
                self.lora_w1 = LoraInjectedLinear(
                    self.w1.in_features,
...
class Attention(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        if self.w_lora:
            if "q_proj" in args.target_modules:
                self.lora_wq = LoraInjectedLinear(
                    self.wq.in_features,
                    self.wq.out_features,
                    lora_r=args.lora_r,
                    lora_alpha=args.lora_alpha,
                    lora_dropout=args.lora_dropout,
                )
            if "k_proj" in args.target_modules:
                self.lora_wk = LoraInjectedLinear(
                    self.wk.in_features,
            ...
            if "v_proj" in args.target_modules:
                self.lora_wv = LoraInjectedLinear(
                    self.wv.in_features,
            ...
            if "o_proj" in args.target_modules:
                self.lora_wo = LoraInjectedLinear(
                    self.wo.in_features,
                    self.wo.out_features,
                    lora_r=args.lora_r,
                    lora_alpha=args.lora_alpha,
                    lora_dropout=args.lora_dropout,
                )
    ...
    def forward(
        ...
        if self.cache_enabled:
            if self.cache_k is None:
                ...
                self.cache_v = torch.cat(
                    [self.cache_v[:, :, :start_pos], values], dim=2
                )
        ...
        output = self._forward_scaled_dot_product_attention(
            xq, keys, values, attention_mask=mask
        )
    ...
    def _forward_scaled_dot_product_attention(self, q, k, v, attention_mask=None):
        if False and hasattr(F, "scaled_dot_product_attention"):
            return F.scaled_dot_product_attention(
                q, k, v, attention_mask if attention_mask is not None else None
            )
...
class Transformer(nn.Module):
        ...
        if self.w_adapter:
            if params.dim == params.adapter_dim:
                ...
                self.down_projection = nn.Sequential(
                    ...
                    nn.ReLU(),
                    nn.Linear(
                        self.params.adapter_dim, self.params.adapter_dim, bias=False
                    ),
                )
    ...
    def forward(self, input_ids, labels, node_ids, attention_mask=None):
        ...
        freqs_cis_prefix = self.freqs_cis.to(inputs_embeds.device)[
    ...
    def transformer_forward(
        ...
        for layer in self.layers:
            h = layer(
                ...
                attention_mask,
                (
                    adapter_key[:, adapter_index].bfloat16(),
                    adapter_value[:, adapter_index].bfloat16(),
                ),
                freqs_cis_prefix,
            )
    ...
    def forward_inference(
        self, adapter, tokens: torch.Tensor, start_pos: int, attention_mask=None
    ):
    ...
    def adapter_forward(self, batch_size, node_ids):
        ...
        if self.adapter_len * self.adapter_layer > 0:
            ...
            edge_index_full, input_node_pair_embed = add_full_rrwp(
                edge_index_sub, num_nodes=len(subset), walk_length=self.rrwp
            )

            adapter_input_ids, adapter_input_attn = (
                self.input_ids[subset],
                self.input_attention_mask[subset],
            )
            ...
            adapter_inputs_embeds = self.graph_adapter_encoder(
                adapter_inputs_embeds, adapter_input_attn
            )

            g_adapter = self.graph_adapter(
                adapter_inputs_embeds,
                adapter_input_attn,
                edge_index_full,
                mapping,
                input_node_pair_embed,
                batch,
            )

            g_adapter = self.up_projection(g_adapter)
            g_adapter = g_adapter.repeat_interleave(
                self.n_layers // self.adapter_layer, dim=1
            )
    ...
    def generate(
        ...
        for cur_pos in range(start_pos, total_len):
            ...
            attention_mask = torch.cat(
                [attention_mask, torch.ones((bsz, 1)).to(attention_mask.device)], dim=-1
            )
...
class PrefixEncoder(nn.Module):
    def __init__(
        ...
        self.prefix_keys = nn.Parameter(
            torch.randn(1, self.adapter_layer, self.adapter_len, self.dim),
            requires_grad=True,
        )
...
class GriTGraphAdapter(nn.Module):
    def __init__(
        ...
        self.query_embed = nn.Parameter(
            torch.randn([self.adapter_layer, self.adapter_len, self.dim])
        )
...
class GritAdapterLayer(nn.Module):
    def __init__(self, params: ModelArgs):
        ...
        self.mp_layer.append(
            MultiHeadAttentionLayerGritSparse(
                0,
                embed_dim=self.dim,
                num_heads=params.adapter_n_heads,
                initial_layer=True,
                rrwp=self.rrwp,
            )
        )
        ...
        self.wp = nn.Sequential(
            nn.Linear(self.rrwp, self.dim), nn.ReLU(), nn.Linear(self.dim, self.dim)
        )
        ...
        self.ind_mlp = nn.Sequential(
            nn.Linear(1, self.dim, bias=False),
            nn.ReLU(),
    ...
    def forward(self, query, edge_index, mapping, input_node_pair_embed, batch):
        ...
        node_pos = node_pos.view([node_pos.shape[0], 1, node_pos.shape[1]]).repeat(
            1, query.shape[1], 1
        )
        ...
        if self.task_level == "graph":
            for query_idx in range(query.shape[1]):
                ...
                for layer in self.mp_layer:
                    x_out, e_out = layer(
                        x=x_out, edge_index=edge_index, input_node_pair_embed=e_out
                    )
            ...
            for query_idx in range(query.shape[1]):
                ...
                x_out, e_out = query[:, query_idx, :], self.wp(input_node_pair_embed)
                ind = torch.zeros([mapping[1] - mapping[0], 1], dtype=query.dtype).to(
                    query.device
                )
                ind[mapping[0] + 1] = 1.0
...
class CrossAttentionLayer(nn.Module):
    ...
    def forward(self, query, input_embeds, input_attn):
        ...
        input_attn = input_attn.masked_fill(
            input_attn.to(torch.bool), torch.finfo(input_embeds.dtype).min
        ).float()

        output = F.scaled_dot_product_attention(
            xq, xk, xv, input_attn if input_attn is not None else None
        )
...
class NeighborContextEncoder(nn.Module):
    def __init__(
        ...
        self.encoder = nn.ModuleList(
            [EncoderLayer(params) for _ in range(params.n_encoder_layers)]
        )
...
class EncoderSelfAttention(nn.Module):
    def __init__(
        ...
        self.freqs_cis = precompute_freqs_cis(
            self.dim // self.n_heads, params.max_seq_len
        )
    ...
    def forward(self, query, input_attn):
        ...
        input_attn = 1.0 - input_attn
        input_attn = input_attn.masked_fill(
            input_attn.to(torch.bool), torch.finfo(query.dtype).min
        ).float()
