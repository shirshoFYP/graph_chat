...
from model.utils import BatchIndices
from tree import InternalParseNode, DUMMY_LABEL, Label
from transition_system import Action


def decode_actions(node_logits, parent_label_logits, new_label_logits, label_vocab):
    """
    ...
    node_idx = torch.argmax(node_logits, dim=-1)
    parent_label_idx = torch.argmax(parent_label_logits, dim=-1)
    new_label_idx = torch.argmax(new_label_logits, dim=-1)
    ...
    return [
        [
            Action(
                (
                    "attach"
                    if label_vocab[new_label_idx[i, j].item()] == DUMMY_LABEL
                    else "juxtapose"
                ),
                target_node=node_idx[i, j].item(),
...
def decode_top_k_actions(
    node_logit, parent_label_logit, new_label_logit, label_vocab, k
):
    """
    Decode top-k actions from action logits for the attach-juxtapose transition system
    """
    node_dist = Categorical(logits=node_logit)
    parent_label_dist = Categorical(logits=parent_label_logit)
    new_label_dist = Categorical(logits=new_label_logit)

    node_log_prob = node_dist.probs.log()
    parent_label_log_prob = parent_label_dist.probs.log()
    new_label_log_prob = new_label_dist.probs.log()
    ...
    for i in range(batch_size):
        ...
        top_log_probs, top_indices = actions_log_prob.view(-1).topk(k)
...
class SequenceDecoder(nn.Module):
    ...
    def forward(self, state: State):
        ...
        node_logits = all_logits[:, :, : self.config.max_sentence_len]
        num_labels = len(self.label_vocab)
        parent_label_logits = all_logits[
            :,
            :,
            self.config.max_sentence_len : (self.config.max_sentence_len + num_labels),
        ]
        new_label_logits = all_logits[
            :, :, (self.config.max_sentence_len + num_labels) :
        ]

        # when in the initial state, parent_label must NOT be () and new_label must be ()
        parent_label_logits[:, 0, 0] = float("-inf")
        new_label_logits[:, 0, 0] = float("-inf")

        action = decode_actions(
            node_logits, parent_label_logits, new_label_logits, self.label_vocab
        )
        actions = [
            action_seq[: len(words)] for action_seq, words in zip(action, tokens_word)
        ]
        logits = {
...
class GraphDecoder(nn.Module):
    ...
    def __init__(self, position_table: torch.Tensor, vocabs: Dict[str, Any], cfg):
        super().__init__()
        ...
        # GCN layers
        self.graph_embedding = GNN(cfg.d_model, cfg.num_gcn_layers)
