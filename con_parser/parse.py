...
class UserProvidedTexts(Dataset):  # type: ignore
    ...
    def __init__(
        ...
        bar = ProgressBar()
        logger.info("Loading input sentences and performing POS tagging..")
        ...
        logger.info("%d input sentences loaded from %s" % (len(self.words), filename))
        ...
        while True:
            ...
            if done:
                ...
                try:
                    ...
                    logger.info("Time elapsed: %f" % (time() - time_start))
                    break

    if cfg.output is not None:
        logger.info("Parse trees saved to %s" % cfg.output)
