...
@torch.no_grad()
def evaluate(model, dataset, split_idx, eval_func, criterion, args, cpu=False):
    model.eval()
    if cpu:
    ...
    if args.dataset in ("deezer-europe", "twitch-e", "fb100"):
...
def evaluate_cpu(model, dataset, split_idx, eval_func, criterion, args, result=None):
    return evaluate(model, dataset, split_idx, eval_func, criterion, args, cpu=True)
