...
def load_fb100_dataset(data_dir, sub_dataset):
    ...
    for col in range(feature_vals.shape[1]):
        ...
        feat_onehot = label_binarize(
            feat_col, classes=np.unique(feature_vals_all[:, col])
        )
...
def load_amazon2m_dataset(data_dir):
    ...
    def load_fixed_splits(train_prop=0.5, val_prop=0.25):
        ...
        if os.path.exists(dir):
            tensor_split_idx["train"] = torch.as_tensor(
                np.loadtxt(dir + "/amazon2m_train.txt"), dtype=torch.long
            )
            tensor_split_idx["valid"] = torch.as_tensor(
                np.loadtxt(dir + "/amazon2m_valid.txt"), dtype=torch.long
            )
            tensor_split_idx["test"] = torch.as_tensor(
                np.loadtxt(dir + "/amazon2m_test.txt"), dtype=torch.long
            )
        else:
            os.makedirs(dir)
            (
                tensor_split_idx["train"],
                tensor_split_idx["valid"],
                tensor_split_idx["test"],
            ) = rand_train_test_idx(
                dataset.label, train_prop=train_prop, valid_prop=val_prop
            )
...
def load_pokec_mat(data_dir):
    """requires pokec.mat"""
    # path.exits -> os.path.exists
    if not os.path.exists(f"{data_dir}pokec.mat"):
        gdd.download_file_from_google_drive(
            file_id=dataset_drive_url["pokec"],
            dest_path=f"{data_dir}pokec.mat",
            showsize=True,
        )
...
def load_snap_patents_mat(data_dir, nclass=5):
    # path.exits -> os.path.exists
    if not os.path.exists(f"{data_dir}snap_patents.mat"):
        p = dataset_drive_url["snap-patents"]
        gdd.download_file_from_google_drive(
            file_id=dataset_drive_url["snap-patents"],
            dest_path=f"{data_dir}snap_patents.mat",
            showsize=True,
        )
...
def load_geom_gcn_dataset(data_dir, name):
    ...
    graph_node_features_and_labels_file_path = (
        f"{data_dir}geom-gcn/{name}/out1_node_feature_label.txt"
    )
